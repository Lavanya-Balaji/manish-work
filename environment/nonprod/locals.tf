locals {
  ...
  primary_sql_server_name = "${lower(var.application)}-${lower(replace(var.primary_location, " ", ""))}-sql-${local.environment_type}"

  secondary_sql_server_name = "${lower(var.application)}-${lower(replace(var.secondary_location, " ", ""))}-sql-${local.environment_type}"
